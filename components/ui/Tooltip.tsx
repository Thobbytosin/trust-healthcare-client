...
const ToolTip = ({ message, position = "top", show }: Props) => {
  ...
  const trianglePosition = {
    ...
    top: "top-full left-12 -translate-x-10 border-t-white",
  ...
  return show ? (
    <div
      className={`absolute w-fit  z-50 text-xs bg-white text-black px-3 py-2 rounded shadow-md ${totalTipPosition} whitespace-pre-line`}
    >
      {message}
      <div
        className={`absolute w-0 h-0 shadow-none  border-8 border-transparent  ${trianglePosition}`}
      />
    </div>
  ) : null;
};
